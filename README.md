...
[`sujet-ai/Sujet-Financial-RAG-EN-Dataset`](https://huggingface.co/datasets/sujet-ai/Sujet-Financial-RAG-EN-Dataset) — corpus de paires questions/réponses/contextes extraites de documents financiers réels, conçu spécifiquement pour l'évaluation de systèmes RAG.

## Architecture

Le pipeline suit une approche RAG classique en 6 étapes :

1. **Chargement des données** — récupération du dataset via la librairie `datasets` de Hugging Face
2. **Préparation des chunks** — extraction des contextes uniques servant de base documentaire
3. **Embeddings** — vectorisation des chunks avec `sentence-transformers` (modèle `all-MiniLM-L6-v2`)
4. **Indexation vectorielle** — stockage des vecteurs dans un index FAISS (recherche par similarité cosinus)
5. **Retrieval** — récupération des k passages les plus pertinents pour une question donnée
6. **Génération** — synthèse de la réponse par un LLM (via OpenRouter), à partir de la question et des passages récupérés
